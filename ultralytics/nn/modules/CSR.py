...
#     改进版：支持分类/回归分支分离的CSR
#     核心修改：
#     1. 添加branch参数，指定注意力作用分支
#     2. alpha参数添加L2正则，防止过度更新
#     3. 弱化空间注意力的强度，减少特征分布破坏
#     """
...
#         self.branch = branch  # cls/reg/all：仅分类分支生效/仅回归分支/全部
...
#         # 改进alpha：初始化为0.1（温和起步）+ 后续加L2正则
#         self.alpha = nn.Parameter(torch.tensor(0.1))  

#     def forward(self, x, is_cls_branch=False):
#         # 如果是回归分支，且指定仅分类分支生效，则直接返回原始特征
...
#         # 2. 空间注意力（弱化强度：分母乘以2，降低energy的幅值）
#         n = w * h - 1
#         x_mean = x.mean(dim=[2, 3], keepdim=True)
#         x_minus_mu_square = (x - x_mean).pow(2)
#         denominator = 8 * (x_minus_mu_square.sum(dim=[2, 3], keepdim=True) / n + dynamic_lambda)  # 分母×2
...
#         # 4. 融合（降低注意力强度：joint_att乘以0.5）
#         joint_att = spatial_att * channel_att * 0.5  # 注意力强度×0.5
