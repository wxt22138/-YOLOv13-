...
class DRFGate(nn.Module):
    """
    动态感受野门控 (Dynamic Receptive Field Gating)
    根据特征激活强度自适应调节大感受野分支权重
    
    原理：小目标区域激活弱但重要 -> 需要更大感受野捕捉上下文
         背景区域激活强但冗余 -> 关闭大感受野节省计算
    """
    def __init__(self, channels, reduction=16):
        ...
        # 可学习的基础权重（初始化为0.5，让网络自己学）
        self.base_weight = nn.Parameter(torch.ones(1) * 0.5)
    
    def forward(self, x):
        # 输出范围 [0, 1]，表示大感受野分支的贡献程度
        return self.gate(x) * self.base_weight.clamp(0, 1)

from .conv import RepVGGBlock,  PartialConv7

class LR2Block(nn.Module):
    """
    LR²-Block: Lightweight Large Receptive-field Reparametrized Block
    轻量级大感受野重参数化模块
    
    核心创新：
    1. 训练时：主路径(3×3) + 稀疏大感受野路径(7×7) 并行
    2. 推理时：大感受野分支被吸收，只剩主路径（零额外开销）
    3. DRFG门控：自适应调节大感受野贡献权重
    ...
    def __init__(self, c1, c2, shortcut=True, e=0.5):
        ...
        # ============ 主路径（训练+推理都存在）============
        self.main_conv1 = Conv(c1, c_, 1)  # 1×1 降维
        self.main_repvgg = RepVGGBlock(c_, c_, 3)  # 可重参数化3×3
        self.main_conv2 = Conv(c_, c2, 1)  # 1×1 升维
        
        # ============ 稀疏大感受野路径（仅训练时启用）============
        self.large_rf_conv = PartialConv7(c1, c_, k=7, s=1)
        self.large_rf_mix = nn.Sequential(
            nn.Conv2d(c_, c_, 3, 1, 1, groups=c_, bias=False),  # 深度卷积混合
            nn.BatchNorm2d(c_),
            nn.SiLU(inplace=True),
            nn.Conv2d(c_, c2, 1, bias=False),  # 1×1 对齐通道
            nn.BatchNorm2d(c2),
        )
        
        # ============ 动态感受野门控 ============
        self.drfg = DRFGate(c1, reduction=16)
        ...
        # 用于存储融合后的权重
        self.fused_large_rf = None
    
    def forward(self, x):
        # 主路径（始终执行）
        main_out = self.main_conv1(x)
        main_out = self.main_repvgg(main_out)
        main_out = self.main_conv2(main_out)
        
        if self.training and not self.deploy:
            # ===== 训练模式：启用大感受野分支 =====
            gate = self.drfg(x)  # [B, 1, 1, 1]
            
            large_out = self.large_rf_conv(x)
            large_out = self.large_rf_mix(large_out)
            
            # 门控加权融合
            out = main_out + gate * large_out
        else:
            # ===== 推理模式：只用主路径 =====
            out = main_out
    ...
    def switch_to_deploy(self):
        ...
        # 1. RepVGG 重参数化
        self.main_repvgg.switch_to_deploy()
        
        # 2. 删除大感受野分支（训练时的知识已经通过梯度传递给了主路径）
        # 注意：这里不是简单删除，而是在训练过程中主路径已经学到了大感受野的模式
        del self.large_rf_conv
        del self.large_rf_mix
        del self.drfg
        
        self.deploy = True
        
        print(f"[LR²-Block] Switched to deploy mode, large RF branch removed")


class C3_LR2(nn.Module):
    """
    使用 LR²-Block 的 C3 模块
    直接替换原有的 C3/C2f/DSC3k2
    """
    def __init__(self, c1, c2, n=1, shortcut=True, e=0.5):
        ...
        self.cv3 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(LR2Block(c_, c_, shortcut, e=1.0) for _ in range(n)))
