...
from ultralytics.nn.modules.SCE import SCE
from ultralytics.nn.modules.CSR import CSR
        ...
        if m in {
            ...
            if m is A2C2f: 
                ...
                if scale in "lx":  # for L/X sizes
                    args.append(True)
                    args.append(1.5)
        elif m in {LR2Block, C3_LR2}:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m is C3_LR2:
                args.insert(2, max(round(n * depth), 1))
                n = 1
        elif m is DySample:
            c1 = ch[f]
            c2 = c1
